...
from .random_crop import random_crop, random_crop_fuse
...
class AdjustBrightness:
    ...
    def __call__(self, img, img_ir, target):
        ...
        img_ir = F.adjust_brightness(img_ir, _brightness_factor)
        return img, img_ir, target
...
class LightingNoise:
    ...
    def __call__(self, img, img_ir, target):
        return lighting_noise(img), lighting_noise(img_ir), target
...
class Rotate:
    ...
    def __call__(self, img, img_ir, target):
        ...
        img_ir, _ = rotate(img_ir, boxes_xyxy, self.angle)
...
class RandomCrop:
    ...
    def __call__(self, img, img_ir, target):
        ...
        try:
            ...
            img, img_ir, new_boxes, new_labels, _ = random_crop_fuse(img, img_ir, boxes_xyxy, labels)
...
class RandomSelectMulti(object):
    ...
    def __call__(self, img, img_ir, target):
        if self.p == -1:
            return random.choice(self.transformslist)(img, img_ir, target)
