...
def random_crop_fuse(image, image_ir, boxes, labels, difficulties=None):
    '''
        image: A PIL image
        boxes: Bounding boxes, a tensor of dimensions (#objects, 4)
        labels: labels of object, a tensor of dimensions (#objects)
    ...
    if type(image) == PIL.Image.Image:
        image = F.to_tensor(image)
    if type(image_ir) == PIL.Image.Image:
    ...
    while True:
        ...
        if mode is None:
            return F.to_pil_image(image), F.to_pil_image(image_ir), boxes, labels, difficulties
        ...
        for _ in range(50):
            ...
            #Crop
            new_image = image[:, int(top):int(bottom), int(left):int(right)] #(3, new_h, new_w)
            new_image_ir = image_ir[:, int(top):int(bottom), int(left):int(right)] #(3, new_h, new_w)
            #Center of bounding boxes
            center_bb = (boxes[:, :2] + boxes[:, 2:])/2.0
            ...
            return F.to_pil_image(new_image), F.to_pil_image(new_image_ir), new_boxes, new_labels, new_difficulties