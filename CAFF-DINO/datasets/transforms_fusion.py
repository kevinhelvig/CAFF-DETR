...
def misalign_img(img, n):
    ...
    img_decalee[:, -n:, :] = 255  # Pour le décalage vers le haut
    img_decalee[:, :, -n:] = 255  # Pour le décalage vers la droite
    """
...
class Misalign_img(object):
    ...
    def __call__(self, img_vis, img_ir, target):
        return misalign_img(img_vis, self.n), img_ir, target #LLVIP style 
        # return img_vis, misalign_img(img_ir, self.n), target #FLIR style
